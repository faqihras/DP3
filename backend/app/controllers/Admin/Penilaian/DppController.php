...
class DppController extends BasicController {
    /**
     ...
     */
     ...
     public function index()
     {
           ...
           $unit=!empty($param['dppUnit'])?$param['dppUnit']:''; 
           $date_now=date('Y',strtotime("1999"));
           $i = 1;
           $query = DB::table('trdpp')
                  ->select('*' ,DB::raw('
                            concat("<button type=\'button\' data-toggle=\'modal\' data-target=\'#myModal\' id=\'btpopup\' onclick=\'detail_data(1,",dppId,")\'><i class=\'fa fa-search\'></i></button>") AS detail
                        ')
                  )

                  // DB::raw('IF(tanggallahir="0000-00-00",concat("-"),tanggallahir) as tanggal_lahir')
                  
                  ->leftjoin('ms_satuankerja','satkerId','=','dppUnit')
                  ->where('dppUnit','=',$unit)
                  ->where('dppJab','=',2) 
                  ->get();
