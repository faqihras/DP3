...
class DpaController extends BasicController {
    /**
     ...
     */
     ...
     public function index()
     {
           ...
           $unit=!empty($param['dpaUnit'])?$param['dpaUnit']:''; 
           $date_now=date('Y',strtotime("1999"));
           $i = 1;
           $query = DB::table('trdpa')
                  ->select('*' ,DB::raw('
                            concat("<button type=\'button\' data-toggle=\'modal\' data-target=\'#myModal\' id=\'btpopup\' onclick=\'detail_data(1,",dpaId,")\'><i class=\'fa fa-search\'></i></button>") AS detail
                        ')
                  )

                  // DB::raw('IF(tanggallahir="0000-00-00",concat("-"),tanggallahir) as tanggal_lahir')
                  
                  ->leftjoin('ms_satuankerja','satkerId','=','dpaUnit')
                  ->where('dpaUnit','=',$unit)
