      ...
      for (i = 0; i <= data.form.length - 1; i++) {
        ...
        if (data.form[i].type == "angka") {
          ...
          result +=
            ...
            "</label>";
          result +=
            '     <input type="text" class="form-control" style="text-align:right" name="' +
            data.form[i].id +
            '" id="' +
            data.form[i].id +
            '"  ' +
        ...
        if (data.form[i].type == "autocomplete") {
          ...
          result +=
            ...
            "</label>";
          result +=
            '    <input type="hidden" class="populate placeholder" name="' +
            data.form[i].id +
            '" id="' +
            data.form[i].id +
            '" >';
        ...
        if (data.form[i].type == "autocomplete") {
          ...
          $("#" + data.form[i].id)
            .on("change", function (e) {
              ...
              if (this.id == "msprogmsskpdKd") {
                var kd = e.added.id;
                // var nmbarang=e.added.text;
                // alert('2222'+kdbarang+'->'+this.id);
                document.getElementById("msprogKd").value = kd + ".";
                // console.log(e);
              }
              if (this.id == "mskegiatanmsprogKd") {
                var kd = e.added.id;
                // var nmbarang=e.added.namabarang;
                // alert('2222'+kdbarang+'->'+this.id);
                document.getElementById("mskegiatanKd").value = kd + ".";
              }
              if (this.id == "mskelompokmsakunKd") {
                var kd = e.added.id;
                // var nmbarang=e.added.namabarang;
                // alert('2222'+kdbarang+'->'+this.id);
                document.getElementById("mskelompokKd").value = kd;
              }

              if (this.id == "msjenismskelompokKd") {
                var kd = e.added.id;
                // var nmbarang=e.added.namabarang;
                // alert('2222'+kdbarang+'->'+this.id);
                document.getElementById("msjenisKd").value = kd;
              }
              if (this.id == "msobjekmsjenisKd") {
                var kd = e.added.id;
                // var nmbarang=e.added.namabarang;
                // alert('2222'+kdbarang+'->'+this.id);
                document.getElementById("msobjekKd").value = kd;
              }

              if (this.id == "robjmsobjekKd") {
                var kd = e.added.id;
                // var nmbarang=e.added.namabarang;
                // alert('2222'+kdbarang+'->'+this.id);
                document.getElementById("robjKd").value = kd;
              }
          ...
          if (idData == "") {
            ...
            if (confirm("Anda Akan Menghapus Data Ini?")) {
              $.ajax({
                ...
                success: function (data3) {
                  newdata();
                  var oTableToUpdate = $("#tData").dataTable({
                    bRetrieve: true,
                  });
          ...
          if (confirm("Simpan Data ini ?")) {
            ...
            for (i = 0; i <= fElement.length - 1; i++) {
              if (fElement[i].type != "file") {
                postData[fElement[i].id] = document.getElementById(
                  fElement[i].id
                ).value;
              }

              if (fElement[i].type == "autocomplete") {
                postData[fElement[i].id + "_hidden"] = document.getElementById(
                  fElement[i].id + "_hidden"
                ).value;
              }
            }
            ...
            $.ajax({
              ...
              success: function (data3) {
                ...
                for (i = 0; i <= fElement.length - 1; i++) {
                  if (fElement[i].type == "file") {
                    ...
                    $.ajax({
                      type: "POST",
                      url: "backend/public/api/admin/setup/uploadTTD",
                      dataType: "json",
                      async: false,
                      data: formdata,
                      processData: false,
              ...
              error: function (XMLHttpRequest, textStatus, errorThrown) {
                alert(
                  textStatus + " : " + errorThrown + " -> Kode Sudah Terpakai"
                );
              },
            });
          }
