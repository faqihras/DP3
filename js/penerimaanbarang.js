...
function setListing() {
    ...
    var table = $('#tDataList').DataTable({
        ...
        "ajax": {
            "url": "backend/public/api/admin/kinerja/kinerja",
            "dataType": "json",
            "data": ({stokhGudang: 1}),
        },
...
function setFieldType() {
    ...
    var field = [
...

        {
            nama :'unit',
            readonly : false,
            apidata : 'backend/public/api/admin/master/unitforcombo',
            type : 'autocomplete',
        },
...
function addList() {
    ...
    result["qty"]           = "<input type='hidden' id='qty" + idxDetail + "' value='" + qty + "'>" + qty;
    // result["hrgbeli"]       = "<input type='hidden' id='hrgbeli" + idxDetail + "' value='" + hrgbeli + "'>" + numeral(hrgbeli).format('0.00');
    result["total"]         = "<input type='hidden' id='total" + idxDetail + "' value='" + total + "'>" + numeral(total).format('0.00');
...
function delList(btn, total){
    ...
    document.getElementById('totaltanpa_ppn').value=numeral(totaltanpa_ppn).format('0,0');
...
function hitTotal() {
    ...
    if (ppn==2){ //cek kondisi exclude
        for(i=1;i<=jmlBrs;i++){
            if(document.getElementById('qty'+i) !== null){
                var jum =document.getElementById('qty'+i).value;
                for(c=1;c<=10;c++){
                    jum = jum.replace(",", "");
                }
            } else {jum = 0;}
    ...
    document.getElementById('totaltrans').value=numeral(jumlah).format('0,0');
    jumlah = 0;
}
...
function simpan() {
   ...
   // var nopenerimaangudang   = document.getElementById('nopenerimaangudang').value;
    ...
    if (lkIdPeg == '') {
        alert("Pegawai Belum dipilih silahkan dipilih terlebih dahulu");
        document.getElementById('isProcessing').value = 0;
        return;
    }
...
function deleteData(idData) {
    var method = 'DELETE';
    var apiUrl = "backend/public/api/admin/kinerja/kinerja/" + idData;
